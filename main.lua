--[[
	Usage:
		local UI = loadstring(...)()  -- or require/dofile
		local win = UI.new({ Title = "My Script", Key = Enum.KeyCode.RightShift })
...
function BWU.new(opts)
	...
	-- Auto-rescale on viewport change
	RunSvc.RenderStepped:Connect(function()
		local ns = getScale()
		if math.abs(usc.Scale - ns) > 0.01 then
			usc.Scale = ns
		end
	end)
...
function BWU:Toggle(force)
	...
	if self._visible then
		...
		task.delay(0.16, function() if not self._visible then win.Visible = false end end)
	end
end

-- ─── Tab ─────────────────────────────────────────────────────────────────────
function BWU:Tab(name)
	...
	local function select()
		...
		for _, t in ipairs(self._tabs) do
			t.sf.Visible = false
			tween(t.btn, 0.12, { BackgroundColor3 = T.tab, TextColor3 = T.subtext })
		end
		sf.Visible = true
		tween(btn, 0.12, { BackgroundColor3 = T.tabSel, TextColor3 = T.text })
		self._curTab = self2
	end
	...
	function self2:Dropdown(ltext, values, default, cb)
		...
		-- dropdown list (rendered outside row)
		local list2 = frame({
		...
		for i, v in ipairs(values) do
			...
			opt.MouseEnter:Connect(function()
				tween(opt, 0.08, { TextColor3 = T.text })
			end)
			opt.MouseLeave:Connect(function()
				tween(opt, 0.08, { TextColor3 = T.subtext })
			end)
		end
	...
	function self2:Button(ltext, cb)
		...
		btn2.MouseEnter:Connect(function()
			tween(btn2, 0.1, { BackgroundColor3 = T.accent })
		end)
	...
	table.insert(self._tabs, { btn = btn, sf = sf })

	-- auto-select first tab
	if #self._tabs == 1 then
		sf.Visible = true
		tween(btn, 0, { BackgroundColor3 = T.tabSel, TextColor3 = T.text })
		self._curTab = self2
	end
